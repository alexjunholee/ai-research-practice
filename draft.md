# AI와 로봇 연구하기 (재구조화 이전 원고)

> 이 파일은 부 중심으로 재구조화하기 전의 원고다. 지금 읽어야 할 본문은
> `part_00_intro.md`부터 `part_03_rules.md`까지이고, `scripts/build_guide.py`도
> 그 넷만 묶는다. 여기 남긴 까닭은 무엇이 어떻게 바뀌었는지 대조할 원본을
> 두기 위해서다. 새 문장을 여기서 가져다 쓰지 않는다.
