...
이 경계와 연결을 정하는 것은 사람의 몫이다. 사람이 그렇게 그어 둔 안에서 자료를 찾아 읽고 옮기는 일을 에이전트가 맡는다. Anthropic의 [2026년 Claude Code 사용 분석](https://www.anthropic.com/research/claude-code-expertise)은 실제 세션에서 이런 분업을 관찰했다. 사용자는 목표, 접근 방식, 완료 기준을 정하는 데 더 많이 개입했고, Claude는 파일 수정과 명령 실행을 더 많이 맡았다. 이 판단을 매번 즉석에서 반복하지 않으려면 에이전트가 따라갈 작업 지침이 필요하다. 이 글의 절차는 저장소의 `AGENTS.md`, project memory, 실험 기록, 주장·근거 표로 옮겨 적을 수 있다.
