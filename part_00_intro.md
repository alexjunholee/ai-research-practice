...
그런 변경 가운데 문서에 공개된 것들은 버전과 수치로 남는다. 버전이 올라가면 수치도 함께 바뀌므로, 이런 값은 본문에 두지 않고 확인한 날짜와 함께 부록의 도구 문서 요약에 모아 두었다.
...
읽은 파일과 실행한 command, 판단의 근거가 된 결과를 기록하면 모델이나 세션이 바뀌어도 작업을 이어 갈 수 있다. 모델의 성능에 따라 탐색 속도와 정리 품질은 달라지지만, 같은 파일과 로그를 보며 같은 절차를 따를 수 있다는 조건은 남는다. 이어지는 세 부에서 이 작업 구조를 하나씩 만든다.
