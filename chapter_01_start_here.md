...
AI는 실제 작업 상태를 자동으로 보증하지 못한다. 현재 process가 살아 있는지, config가 runtime에서 읽혔는지, metric script가 같은 조건을 썼는지, reviewer가 공격한 주장에 답할 만큼 근거가 있는지는 파일과 실행 결과를 봐야 한다.
...
반복 실패의 원인은 주로 작업 구조에 있다. AI는 대화와 파일 일부를 보고 가능한 다음 설명을 만든다. 로보틱스/CV 연구는 대화 밖의 상태를 바꾼다. Docker container, ROS2 topic, CUDA process, dataset split, calibration file, metric script, TeX table은 실제 파일과 실행 결과로 확인한다.

AI가 강한 공간은 텍스트와 구조다. 연구자는 실행과 증거를 확인한다. 이 둘을 섞으면 같은 오류가 반복된다.
...
Hutchins의 distributed cognition은 판단이 사람 머리 안에서만 이루어지지 않는다고 본다. 연구실의 판단도 repo, launch file, dataset, terminal output, paper table, 기억 노트에 분산된다. AI가 말한 연결이 그럴듯해도, 이 자료들을 다시 대조해야 연구 상태를 판단할 수 있다.
