...
| Harness | 상태, 증거, 행동, 확인, claim을 연결하는 운영 구조 | harness는 연구 상태 전이를 관리하는 규칙이다 |
...
| Terminal/script | build, metric 산출, checksum 확인 | 실행 command와 artifact path를 남긴다 |
...
Codex에서는 `AGENTS.md`, `templates/`, project memory가 이식된 규칙을 받는다.
이식 후에는 `codex-porting-checklist.md`로 빠진 gate를 확인한다.
