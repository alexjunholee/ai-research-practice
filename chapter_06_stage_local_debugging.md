...
AI는 root cause를 빨리 말한다. QoS, calibration, cache, normalization 같은 단어가 바로 나온다. 도움이 될 때도 있다. 어느 단계에서 신호가 끊겼는지 확인하기 전까지는 가능한 설명으로 둔다.
...
build pass는 소스 컴파일만 말한다. callback, tf lookup, metric correctness, trajectory validity는 따로 확인한다.
