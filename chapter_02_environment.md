...
이 칸들이 어디에 놓이느냐도 함께 정해진다. 반복되는 실패 유형과 운영 규칙과 공개용 template은 공개 저장소에 둔다. 개인 대화 원문과 개인 경로와 reviewer 원문과 미공개 숫자와 인증 정보는 로컬 기록에 남긴다. 부록 D가 이 경계를 다시 시작할 때 확인할 것으로 꼽아 두었다.
...
부록 B는 첫날 체크리스트와 함께 첫 세션에 넣을 프롬프트도 두었는데, 거기 이 한 줄이 들어 있다.
...
둘째 줄까지가 읽는 일이고 셋째부터 돌려 보는 일이다. 요약은 어느 파일을 열지 정하는 데까지 쓴다. 열어 보니 요약이 말한 것과 다른 것이 들어 있으면 여섯째 줄로 간다. 그때 어긋난 자리를 새로 적는 다섯 줄 옆에 남기고 요약은 그대로 둔다. 요약을 고쳐 두면 다음번에 무엇이 어긋났는지 대조할 원본이 없어진다. 같은 자리에서 또 어긋나면 첫날 체크리스트의 `durable corrections`로 올린다.

셋째와 넷째 줄에서 지난번과 어긋난 것을 찾는다. 원 파일을 찾을 때 config와 split과 calibration file을 함께 열고, 명령과 output path를 볼 때 그 명령이 어느 container에서 어떤 environment로 돌았는지 본다. 코드가 그대로여도 split이 바뀌었으면 같은 조건이 아니다.
