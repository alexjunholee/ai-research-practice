...
| 연구 장면 | 먼저 열 도구 표면 | 첫 gate |
|---|---|---|
| repo를 읽고 작은 수정을 한다 | coding agent | `AGENTS.md`와 source file을 먼저 읽었는가 |
| 논문 claim을 code와 experiment에 연결한다 | chat model 또는 coding agent | paper-code-experiment map이 남는가 |
| ROS2, Docker, CUDA, dataset 오류를 좁힌다 | coding agent와 terminal | stage-local command outcome이 있는가 |
| 외부 repo나 논문을 찾는다 | browser/search tool | 출처 URL과 claim boundary가 분리됐는가 |
| 원고와 rebuttal을 고친다 | manuscript operator role | allowed wording과 blocked wording이 분리됐는가 |
| 반복 실패를 막는다 | harness operator role | ledger나 replay case로 승격했는가 |
...
옮길 규칙과 버릴 명령을 나눈다. 파일 이름이나 plugin 명령보다 중요한 것은
...
이 요청은 발산한 후보가 연구 행동으로 바뀌기 전에 증거 상태를 붙인다.
...
AI가 틀린 assumption을 했다면 `replay-case.md`로 승격한다. 같은 실수를
...
memory가 바뀌지 않으면 다음 AI 세션은 같은 proof boundary를 이어받지 못한다.
