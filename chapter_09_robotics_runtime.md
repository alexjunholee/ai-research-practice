...
AI 코딩 에이전트(Claude, Copilot, ChatGPT 등)는 함수의 초안을 만들고 에러 메시지에서 원인 후보를 찾는 데 유용하다. 로보틱스에서는 하드웨어, OS, 네트워크, 실시간 조건이 함께 맞물리므로 원인은 코드 밖에서도 찾아야 한다. 현재 장치와 실행 상태를 함께 보여 줘야 에이전트의 답이 지금 이 로봇을 가리킨다.
